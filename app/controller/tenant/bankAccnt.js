...
exports.createBankAccnt = async (req, res) => {
  ...
  try{
    if (!employeeId || !accountHolderName || !bankName || !bankBranch || !accountNumber) {
        return Helper.response(false, "All fields are required", null, res, 400); 
    }

    if (!Helper.validateAccountNumber(accountNumber)) {
      return Helper.response(false, "Account number must be between 11 to 14 digits", null, res, 400);
    }
    ...
    const existingAccount = await bankAccnt.findOne({
      where: { tenantId, employeeId, accountNumber }
    });

    if (existingAccount) {
      return Helper.response(false, "Bank account already exists for this employee", null, res, 409);
    }
    ...
    return Helper.response(true, "Bank account created successfully", newBankAccnt, res, 201);
  }
  catch(err)
  {
...
exports.getBankAccnt = async (req, res) => {
    ...
    try {
        ...
        return Helper.response(true, "Bank accounts retrieved successfully", bankAccounts, res, 200);
    } catch (err) {
...
exports.updateBankAccnt = async (req, res) => {
    ...
    try {
        ...
        const bankAccount = await bankAccnt.findOne({ where: { id, employeeId, tenantId } });
        ...
        return Helper.response(true, "Bank account updated successfully", bankAccount, res, 200);
    } catch (err) {
...
exports.deleteBankAccnt = async (req, res) => {
    ...
    try {
        ...
        if (!id || !employeeId) {
            return Helper.response(false, "ID and Employee ID are required", null, res, 400);
        }
        ...
        return Helper.response(true, "Bank account deleted successfully", null, res, 200);
    } catch (err) {
